import type { ReactNode } from 'react';
import clsx from 'clsx';
import { Content } from '@utils/content';
import { baseProps, isHtmlString } from '@utils/functions';
import type { Base } from '@utils/types';
import cn from '@components/warranty/warranty.module.css';

const WarrantyIcon = () => (
  <svg
    data-testid="warranty-icon"
    focusable="false"
    role="img"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 48 56"
    className={cn.Icon}
  >
    <title>Warranty Icon</title>
    <g fill="currentColor">
      <path d="M23.373 0H24.38c3.56.001 7.111.38 10.592 1.131a51.213 51.213 0 0 1 10.18 3.35h.006a4.569 4.569 0 0 1 2.59 4.167v17.98a31.89 31.89 0 0 1-1.404 9.045 30.786 30.786 0 0 1-3.784 8.001 29.024 29.024 0 0 1-5.83 6.444 27.225 27.225 0 0 1-7.532 4.359l-2.44.97a7.803 7.803 0 0 1-5.766 0l-2.442-.971a27.225 27.225 0 0 1-7.533-4.359 29.023 29.023 0 0 1-5.828-6.445 30.787 30.787 0 0 1-3.784-8.002A31.892 31.892 0 0 1 0 26.626V8.654a4.57 4.57 0 0 1 2.594-4.17H2.6a51.212 51.212 0 0 1 10.18-3.35A50.314 50.314 0 0 1 23.372 0Zm.504 1.898h-.503a48.406 48.406 0 0 0-10.195 1.089A49.31 49.31 0 0 0 3.38 6.214a2.666 2.666 0 0 0-1.483 2.443v17.955A29.392 29.392 0 0 0 6.77 42.627a26.093 26.093 0 0 0 12.462 10.078l.014.005 2.448.974a5.905 5.905 0 0 0 4.366 0l2.446-.973.014-.005a26.093 26.093 0 0 0 12.46-10.079 29.392 29.392 0 0 0 4.871-16.01V8.654a2.666 2.666 0 0 0-1.482-2.442 49.31 49.31 0 0 0-9.795-3.225 48.415 48.415 0 0 0-10.196-1.089h-.5Z" />
      <path d="M14.166 40.145h-3.523a1.504 1.504 0 0 1-1.536-1.537v-8.22a1.505 1.505 0 0 1 1.536-1.536h3.523a1.505 1.505 0 0 1 1.537 1.537v8.22a1.504 1.504 0 0 1-1.537 1.536Zm-3.16-1.898h2.799v-7.499h-2.8v7.499Z" />
      <path d="M24.384 41.254c-1.553 0-3.163-.57-6.075-1.591a239.187 239.187 0 0 0-3.855-1.329.95.95 0 0 1 .6-1.803c1.528.508 2.78.949 3.885 1.338 2.99 1.052 4.492 1.58 5.82 1.467 1.328-.112 2.782-.892 5.664-2.443 1.32-.71 2.905-1.563 4.907-2.583a2.297 2.297 0 0 0-1.503-.082l-5.192 1.725a.95.95 0 1 1-.598-1.803l5.207-1.728.03-.01a4.209 4.209 0 0 1 4.556 1.519.949.949 0 0 1-.337 1.415c-2.613 1.307-4.584 2.367-6.169 3.22-3.114 1.676-4.676 2.517-6.404 2.663a5.686 5.686 0 0 1-.536.025Z" />
      <path d="M27.297 36.64c-.095 0-.19-.006-.285-.018h-6.387a.95.95 0 0 1 0-1.899h6.454c.057 0 .113.005.168.016a.245.245 0 0 0 .287-.24.244.244 0 0 0-.287-.24.955.955 0 0 1-.168.014h-3.522a.95.95 0 0 1-.681-.284 6.76 6.76 0 0 0-4.62-2.06h-3.502a.95.95 0 0 1 0-1.898h3.556a8.645 8.645 0 0 1 5.64 2.347h3.062a2.143 2.143 0 1 1 .285 4.266v-.003ZM27.464 14.006a.95.95 0 0 1-.625-.234l-3.635-3.18a.949.949 0 0 1 .095-1.505l4.096-2.725a.95.95 0 0 1 1.053 1.58l-3.061 2.037 2.701 2.363a.95.95 0 0 1-.624 1.664Z" />
      <path d="M24.737 28.65a9.966 9.966 0 0 1-6.348-17.649l1.1 1.56a8.068 8.068 0 1 0 5.248-1.944c-.124 0-.25 0-.38.008l-.174-1.892a9.966 9.966 0 1 1 .554 19.917Z" />
      <path d="M18.947 12.723a.95.95 0 1 0 0-1.899.95.95 0 0 0 0 1.899ZM25.99 20.948a1.459 1.459 0 0 1-1.116.452 1.106 1.106 0 0 1-1.13-.983c-.013-.08 0-.133.12-.133h1.289a.522.522 0 0 0 .518-.585.498.498 0 0 0-.518-.57H23.73c-.095 0-.12-.026-.12-.066v-.427c0-.095.014-.12.107-.12h1.435a.522.522 0 0 0 .518-.584.497.497 0 0 0-.518-.57h-1.316c-.095 0-.12-.053-.095-.146a1.082 1.082 0 0 1 1.132-.958 1.33 1.33 0 0 1 1.076.438.664.664 0 0 0 .439.133.962.962 0 0 0 .854-.864.91.91 0 0 0-.452-.73 2.968 2.968 0 0 0-1.913-.598c-1.927 0-2.563 1.156-2.83 2.604-.014.08-.054.12-.12.12h-.293a.5.5 0 0 0-.518.569.522.522 0 0 0 .518.585h.2c.066 0 .106 0 .106.12v.371c0 .067 0 .12-.12.12h-.19a.5.5 0 0 0-.518.57.521.521 0 0 0 .518.584h.293c.066 0 .106.027.12.133.252 1.462.93 2.591 2.83 2.591a2.775 2.775 0 0 0 2.285-.93.746.746 0 0 0 .253-.57.99.99 0 0 0-.877-.823.766.766 0 0 0-.545.267Z" />
    </g>
  </svg>
);

export interface WarrantyProps extends Base {
  title: ReactNode;
  text: ReactNode;
}

/**
 * Displays a warranty badge with icon, title, and descriptive text.
 */
export const Warranty = ({ title, text, ...base }: WarrantyProps) => (
  <div
    data-testid={baseProps(base, 'data-testid', 'warranty')}
    className={clsx(cn.Warranty, baseProps(base, 'className'))}
  >
    <WarrantyIcon />
    <div className={cn.Main}>
      {isHtmlString(text)}
      <Content data-testid="warranty-title" className={cn.Title}>
        {title}
      </Content>
      <Content data-testid="warranty-text" className={cn.Text}>
        {text}
      </Content>
    </div>
  </div>
);
